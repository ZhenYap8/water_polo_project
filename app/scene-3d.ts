...
import {createSwimmerFactory} from './swimmer';
...
export type CameraMode='overhead'|'broadcast'|'endline'|'firstperson';
...
export function createPoolScene(canvas:HTMLCanvasElement){
 ...
 const swimmerFactory=createSwimmerFactory();const swimmers:any[]=[];
 for(let team=0;team<2;team++)for(let i=0;i<7;i++){const rig=swimmerFactory.create(skin[(i+team)%skin.length],i===0?red:team===0?white:blue,team===0?blue:red,white,team*7+i);scene.add(rig.root);
 ...
 const label=textSprite(String(i+1),team===0?'#f1ffff':'#ffc4b9');scene.add(label);swimmers.push({...rig,ripple,label});}
 ...
 const raycaster=new THREE.Raycaster(),plane=new THREE.Plane(new THREE.Vector3(0,1,0),0),intersection=new THREE.Vector3();let mode:CameraMode='overhead',aspect=1,lastT=-1,yaw=Math.PI/2,pitch=-.035;let tracked={x:0,z:0};
 function resize(){const r=canvas.getBoundingClientRect();if(r.width<1||r.height<1)return;aspect=r.width/r.height;renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,1.6,Math.sqrt(2400000/(r.width*r.height))));renderer.setSize(r.width,r.height,false);camera.aspect=aspect;camera.updateProjectionMatrix();if(mode!=='firstperson')fitCamera(camera,mode,aspect);}
 const observer=new ResizeObserver(resize);observer.observe(canvas);resize();
 function update(g:any){
 const now=g.elapsed,selected=g.players[g.selected];const wakes=g.players.map((p:any)=>({x:(p.x/W-.5)*25,z:(p.y/H-.5)*20,dx:p.facingX,dz:p.facingY,strength:.016+Math.min(.06,Math.hypot(p.vx/44,p.vy/31)*.016)+(p.goalkeeping?.jump||0)*.09,phase:p.id}));
 // All cameras see the entire pitch. The low end-line view looks towards Blue's attack.
 if(mode==='firstperson'){const position=world(selected.x,selected.y);tracked={x:position.x,z:position.z};swimmerView(camera,position.x,position.z,yaw,pitch);}else camera.updateMatrixWorld();
 if(now<lastT||now-lastT>=1/30){for(let i=0;i<wp.count;i++){const x=base[i*3],z=base[i*3+2];wp.setY(i,waterHeight(x,z,now,wakes));}wp.needsUpdate=true;waterGeometry.computeVertexNormals();lastT=now;}
 for(let i=0;i<g.players.length;i++){const p=g.players[i],sw=swimmers[i];if(!sw)continue;const location=world(p.x,p.y);sw.root.visible=!(mode==='firstperson'&&p.id===g.selected);sw.root.position.copy(location);sw.root.position.y=waterHeight(location.x,location.z,now,wakes)+(p.keeper?(p.goalkeeping?.jump||0):.012*Math.sin(now*3+p.id));const angle=Math.atan2(-p.facingX*25/W,-p.facingY*20/H);const moving=Math.hypot(p.vx,p.vy)>8;sw.body.rotation.y=angle;const hold=g.ball.owner===p.id;sw.animate(now,Math.hypot(p.vx,p.vy),p.keeper,hold,g.charge,p.goalkeeping);sw.ripple.position.copy(location);sw.ripple.position.y=waterHeight(location.x,location.z,now)+.045;const scale=1+.18*Math.sin(now*4+i)+(p.goalkeeping?.jump||0)*1.2;sw.ripple.scale.set(scale,moving?scale*1.3:scale,1);sw.label.position.copy(location).add(new THREE.Vector3(0,.85+(p.goalkeeping?.jump||0),0));sw.label.visible=mode==='firstperson'?p.id!==g.selected:mode==='overhead'||p.id===g.selected||hold;}
 ...
 const bp=world(g.ball.x,g.ball.y);ball.position.copy(bp);ball.position.y=g.ball.owner!==null?waterHeight(bp.x,bp.z,now)+.25+(g.charge*.65):(g.ball.height??.09);if(g.ball.owner!==null&&g.charge>.05){const carrier=swimmers[g.ball.owner];if(carrier){carrier.root.updateMatrixWorld(true);carrier.hand.getWorldPosition(ball.position);}}if(mode==='firstperson'&&g.ball.owner===g.selected){const forward=camera.getWorldDirection(new THREE.Vector3()),right=new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld,0);ball.position.copy(camera.position).addScaledVector(forward,.52).addScaledVector(right,.18);ball.position.y-=.23;}ball.rotation.x=now*3;ball.rotation.z=now*4;shadow.position.copy(bp);shadow.position.y=.06;shadow.scale.setScalar(1+ball.position.y*.2);
