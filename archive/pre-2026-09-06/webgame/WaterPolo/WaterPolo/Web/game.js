...
function resizeCanvas() {
  ...
  // Real pixel buffer for crisp drawing
  canvas.width  = Math.round(rect.width  * dpr);
  canvas.height = Math.round(rect.height * dpr);

  // World units match CSS pixels
...
class Entity {
  constructor(x,y,r,color,team){
    ...
    this.maxSpeed=5; this.accel=0.8; this.friction=0.90;
  ...
  draw(){
    // Outer ring: red for goalies; otherwise team-tinted ring
    ctx.beginPath();
    ctx.arc(this.x, this.y, this.r + 3, 0, Math.PI * 2);
    ctx.lineWidth = 2;
    if (this.isKeeper) {
      ctx.strokeStyle = this.color; // match goalie red
    } else {
      ctx.strokeStyle = (this.team === 'blue') ? '#bfe7ff' : '#e0e0e0';
    }
    ctx.stroke();
...
const players = [], blueFieldPlayers = [], whiteFieldPlayers = [];
const ball = new Ball();
...
function spawnTeams(){
  players.length=0; blueFieldPlayers.length=0; whiteFieldPlayers.length=0;

  // Blue field players (darker blue)
  for(let i=0;i<6;i++){
    const b = new Entity(W*0.25, H*(0.2+0.12*i), 15, '#0a3d91','blue'); // dark blue
    if(i===0) b.ai=false;
    players.push(b); blueFieldPlayers.push(b);
  }
  // Blue goalie - darker red
  const gkB = new Entity(poolPadding+goalDepth+24, H*0.5, 18, '#8B0000','blue');
  gkB.isKeeper=true; players.push(gkB);

  // White field players
  for(let i=0;i<6;i++){
    const w = new Entity(W*0.75, H*(0.2+0.12*i), 15, '#FFFFFF','white');
    players.push(w); whiteFieldPlayers.push(w);
  }
  // White goalie - bright red
  const gkW = new Entity(W-(poolPadding+goalDepth+24), H*0.5, 18, '#FF0000','white');
  gkW.isKeeper=true; players.push(gkW);
...
function aiThink(){
  const blueD = formDShape("blue"), whiteD = formDShape("white");

  // --- WHITE TEAM ATTACK BRAIN (one-time per frame) ---
  if (ball.holder && ball.holder.team === 'white' && ball.holder.ai) {
  ...
  for (const pl of players) {
    ...
    const teamArray = pl.team==='blue' ? blueFieldPlayers : whiteFieldPlayers;
    const shape = pl.team==='blue' ? blueD : whiteD;
    ...
    if (ball.holder && ball.holder.team === pl.team) {
      ...
      const opps = pl.team==='blue' ? whiteFieldPlayers : blueFieldPlayers;
...
function goalCheck(){
  ...
  if(!inY) return;
  if(ball.x < poolPadding){ score.white++; resetKickoff('white'); }
...
function setTeamKickoffPositions(team) {
  ...
  const fields = isBlue ? blueFieldPlayers : whiteFieldPlayers;
...
function resetKickoff(lastScoredTeam){
  ...
  setTeamKickoffPositions('white');

  const concedingTeam = lastScoredTeam === 'blue' ? 'white' : 'blue';
  const starters = concedingTeam === 'blue' ? blueFieldPlayers : whiteFieldPlayers;
...
function step(dt){
  ...
  document.getElementById('score').textContent = `Blue ${score.blue} — ${score.white} White`;
...
function loop(now){
  ...
  last = now;
  if (running) {
    step(dt);        // update only when running
  }
  render();          // always draw current frame
...
canvas.addEventListener('mousemove', (e)=>{
  const rect = canvas.getBoundingClientRect();
  mouse.x = e.clientX - rect.left;   // <- no extra scaling
  mouse.y = e.clientY - rect.top;    // <- no extra scaling
});
...
window.addEventListener('mouseup', ()=>{
  if (mouse.down && ball.holder === getControlledBlue()) {
    tryShoot(getControlledBlue(), mouse.x, mouse.y, mouse.power);
    mouse.power = 0;
  }
  mouse.down = false;
});
